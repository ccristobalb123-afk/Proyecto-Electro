import { IconFile, IconEye, IconUpload, IconTrash } from "../icons/Icons";
import { ARCHIVOS_BASE_URL } from "../../services/apiClient";
import Loading from "../shared/Loading";
import EstadoVacio from "../shared/EstadoVacio";
import EstadoError from "../shared/EstadoError";

export default function TablaRrhh({ cargando, error, onReintentar, items, tab, estadoLabel, hayFiltrosActivos, onNuevo, onVerDetalle, onEditar, onEliminar, onAdjuntarArchivo }) {
            ...
            {items.map((item) => (
              <tr key={item.id}>
                ...
                {tab === "contratos" && (
                  <td>
                    {item.archivo ? (
                      <a className="doc-chip" href={`${ARCHIVOS_BASE_URL}${item.archivoUrl}`} target="_blank" rel="noreferrer">
                ...
                <td>
                  <div className="rrhh-tabla-acciones">
                    <button className="btn-outline-sm" type="button" onClick={() => onEditar(item)}>Editar</button>
                    <button className="icon-btn" type="button" title="Ver detalle" onClick={() => onVerDetalle(item)}>
                      <IconEye />
                    </button>
                    <button className="icon-btn" type="button" title="Eliminar" onClick={() => onEliminar(item)}>
                      <IconTrash />
                    </button>
                  </div>
                </td>
        ...
        {items.map((item) => (
          <div className="rrhh-mobile-card" key={item.id}>
            <div className="rrhh-mobile-card-top">
              ...
              <div className="rrhh-mobile-card-acciones">
                <button className="btn-outline-sm" type="button" onClick={() => onEditar(item)}>Editar</button>
                <button className="icon-btn" type="button" title="Ver detalle" onClick={() => onVerDetalle(item)}>
                  <IconEye />
                </button>
