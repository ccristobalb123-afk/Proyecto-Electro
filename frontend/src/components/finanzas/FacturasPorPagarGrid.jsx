...
import { totalPagado, estadoPago, montoNetoAPagar } from "../../services/facturasService";
import { soles, formatFecha, ESTADO_PAGO_LABEL } from "./finanzasUtils";

export default function FacturasPorPagarGrid({ cargando, error, onReintentar, facturas, onNuevo, onSubirComprobante, onRegistrarPago, onEditar, onAnular }) {
  ...
  return (
    <div className="fin-grid">
      {facturas.map((f) => {
        ...
        const saldo = montoNetoAPagar(f) - totalPagado(f.pagos);
        return (
          <div className="fin-card" key={f.id}>
            ...
            <div className="fin-card-body">
              ...
              {f.catalogoDetraccion && (
                <span className="fin-tag-detraccion">
                  {f.catalogoDetraccion.codigo} · {f.catalogoDetraccion.descripcion} — S/ {f.montoDetraccion.toFixed(2)}
                </span>
              )}

              {f.archivo ? (
                <a className="fin-card-archivo fin-card-archivo--link" href={`${ARCHIVOS_BASE_URL}${f.archivoUrl}`} target="_blank" rel="noreferrer">
                ...
                <label className="fin-card-archivo">
                  ...
                  <input type="file" accept=".pdf,image/*" className="input-oculto" onChange={(e) => e.target.files[0] && onSubirComprobante(f.id, e.target.files[0])} />
                </label>
              )}
            </div>
            ...
            <div className="fin-card-footer">
              ...
              <button
                className="icon-btn"
                type="button"
                title={f.anulada ? "Ya está anulada" : "Anular factura por pagar"}
                disabled={f.anulada}
                onClick={() => onAnular(f)}
              >
